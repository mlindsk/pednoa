...
#'   list(1, c(2, 3, 4), list(c(2, 3), c(1, 1), c(2, 3)))
#' )
#'
#' # Individual 3 has genotype (2,2) on loci 1 and (1,1) on loci two.
#' # Individual 1 has genotype (2,3) on loci 2, (1,1) on loci 3 and (2,3) on loci 4 
#'
#'
...
#' # probability of seeing 4,3,...,16 different alleles over four locis for 1 and 5
#' # Note: They cannot have 16 different alleles due to the evidence.
#' 
#' p <- pednoa(
#'   list(p1, p2, p3, p4),
#'   c(1, 5),
...
pednoa <- function(
  ...
  if (!is_whole_number(mvec)) {
    stop("mvec must be a vector of integral numbers (integers)")
  }

  if (!is_whole_number(founder_vec)) {
    stop("founder_vec must be a vector of integral numbers (integers)")
  }
  ...
  pmfs <- lapply(seq_along(afreq_list), function(l) {
    ...
    el <- e[[l]]
    pednoa_engine(afreq_l, mvec, founder_vec, nonfounder_list, el, trace)
  })
